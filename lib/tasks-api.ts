...
export async function createTask(data: {
  ...
  if (error) throw error
  return { ...task, status: task.status as Task['status'], priority: task.priority as Task['priority'], subtasks: [] }
}
...
export async function updateTask(
  ...
  const { subtasks: _s, assignee: _a, ...dbPatch } = patch as Record<string, unknown>
